...
if args.save and 'SNN' in args.model_type:
    ...
    logging.info("#### Saving SNN model ####")
    # 1. Save original SNN weights
    model.save_weights(args.logging_dir + '/' + args.model_name + '_snn_weights.h5')
    logging.info('saved original SNN model weights')

    # 2. Save preprocessed SNN model if necessary
    # Add any preprocessing steps here if needed
    model.save_weights(args.logging_dir + '/' + args.model_name + '_snn_preprocessed.h5')
    logging.info('saved preprocessed SNN model')
...
if 'SNN' in args.model_type:
    ...
    if 'SNN' in args.model_type and args.testing:

        print("#######################################################")
        print("loading SNN model weights for testing")
        model.load_weights(args.logging_dir + '/' + args.model_name + '_snn_weights.h5')
        logging.info('Loaded SNN model weights for testing')

    # Evaluate the model
    test_acc = model.evaluate(data.x_test, data.y_test, batch_size=args.batch_size)
    logging.info("Testing accuracy is {}.".format(test_acc))
